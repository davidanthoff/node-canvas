...
// note: do NOT access `this` since it may not be a canvas instance
...
parsePNGArgs(Napi::Value arg, PngClosure& pngargs) {
  if (arg.IsObject()) {
    ...
    if (obj.Get("palette").UnwrapTo(&palette) && palette.IsTypedArray()) {
      ...
      pngargs.palette = static_cast<uint8_t*>(palette_ta.ArrayBuffer().Data());
...
Canvas::ToBuffer(const Napi::CallbackInfo& info) {
  ...
  if (name == "pdf" || name == "svg") {
    ...
    return Napi::Buffer<uint8_t>::New(env, &closure->vec[0], closure->vec.size());
  }
...
streamPNG(void *c, const uint8_t *data, unsigned len) {
  ...
  Napi::Object buf = Napi::Buffer<uint8_t>::Copy(env, data, len);
