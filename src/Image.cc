...
// note: do NOT access `this` since it may not be an image instance
#define CHECK_RECEIVER() if (info.Env().IsExceptionPending()) return info.Env().Undefined();
#define CHECK_RECEIVER_VOID() if (info.Env().IsExceptionPending()) return;
...
Image::GetSource(const Napi::CallbackInfo& info){
  ...
  InstanceData *data = env.GetInstanceData<InstanceData>();
  if (!info.This().As<Napi::Object>().InstanceOf(data->ImageCtor.Value()).UnwrapOr(false)) {
    // #1534
    if (!env.IsExceptionPending()) {
      Napi::Error::New(env, "Method Image.GetSource called on incompatible receiver").ThrowAsJavaScriptException();
...
Image::SetSource(const Napi::CallbackInfo& info){
  ...
  if (!info.This().As<Napi::Object>().InstanceOf(data->ImageCtor.Value()).UnwrapOr(false)) {
    // #1534
    if (!env.IsExceptionPending()) {
      Napi::Error::New(env, "Method Image.SetSource called on incompatible receiver").ThrowAsJavaScriptException();
      return;
    }
  }
...
clearMimeData(void *closure) {
  ...
  free(closure);
  delete static_cast<read_closure_t *>(closure)->env; 
}
...
Image::assignDataAsMime(uint8_t *data, int len, const char *mime_type) {
  ...
  mime_closure->env = new Napi::Env(env);
